...
- timeToUpdate - время в мс, указывающее на то, как часто следует делать проверки на устаревание данных
...
const mobxQuery = new MobxQuery({
    ...
    timeToUpdate: 60000 // 1мин по умолчанию, опционально
});
```
...
При создании квери можно указать, через какое время данные станут неактуальными
```ts
const query = mobxQuery.createQuery(
    ['key one'],
    () => Promise.resolve('foo'),
    { 
        timeToLive: 10000 // по умолчанию не задается, и проверки не запускаются
    }
);
```
